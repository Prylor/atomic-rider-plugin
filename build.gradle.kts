...
sourceSets {
    main {
        ...
        java.srcDir("src/rider/main/gen")
...
tasks {
    val generateAtomicLexer by registering(org.jetbrains.grammarkit.tasks.GenerateLexerTask::class) {
        sourceFile.set(file("src/rider/main/kotlin/com/jetbrains/rider/plugins/atomic/language/Atomic.flex"))
        targetOutputDir.set(file("src/rider/main/gen/com/jetbrains/rider/plugins/atomic/language"))
    }
    
    val generateAtomicParser by registering(org.jetbrains.grammarkit.tasks.GenerateParserTask::class) {
        sourceFile.set(file("src/rider/main/kotlin/com/jetbrains/rider/plugins/atomic/language/Atomic.bnf"))
        targetRootOutputDir.set(file("src/rider/main/gen"))
        pathToParser.set("com/jetbrains/rider/plugins/atomic/parser/AtomicParser.java")
        pathToPsiRoot.set("com/jetbrains/rider/plugins/atomic/psi")
        purgeOldFiles.set(true)
    }
    
    compileKotlin {
        kotlinOptions { jvmTarget = "17" }
        dependsOn(generateAtomicLexer, generateAtomicParser, ":protocol:rdgen")
    }
...
tasks.prepareSandbox {
    dependsOn(compileDotNet, ":protocol:rdgen")
